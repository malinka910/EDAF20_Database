...
public class BookingTab {
	...
	private void fillNamesList() {
		...
		//This program initializes the gui before it creates a db object
		if(db==null){	
			//empty list to prevent db problem with dates function too
			//allmovies.add("Login for movies");
		}else{
			try {
				allmovies.addAll(db.getMovies());
